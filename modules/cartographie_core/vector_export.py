...
def prepare_layer_export_data(rows, layer_key, config):
    """
    Prepare DataFrame for a layer by cleaning and renaming columns.
    ...
    Returns:
        pd.DataFrame: Cleaned DataFrame ready for export, or None if empty
    """
    if not rows:
        return None
...
def generate_unified_excel(layers_data, folder_path, filename="Export_Cartographie.xlsx"):
    """
    Generate a single Excel file with multiple sheets (one per layer).

    Args:
        layers_data (dict): Dictionary {layer_key: pd.DataFrame}
        folder_path (str): Output directory path
        filename (str): Name of the output file

    Returns:
        str: Path to generated Excel file, or None if failure
    """
    if not layers_data:
    ...
    logger.info(f"[Export] Generating unified Excel: {full_path}")
    ...
    try:
        ...
        header_fmt = workbook.add_format({
            'bold': True, 'fg_color': '#005b9e', 'font_color': 'white', 
        ...
        warning_fmt = workbook.add_format({
            'border': 1, 'text_wrap': True, 'valign': 'vcenter', 'align': 'center',
            'bg_color': '#fff9c4', 'font_color': '#b45309'
        })

        for layer_key, df in layers_data.items():
            # Sheet name limit is 31 chars
            sheet_name = layer_key[:31] 
            ...
            center_cols = {"Référence", "Etat d'occupation du site", "Code Postal", "Numéro Parcelle", "Section"}
        ...
        logger.error(f"[Export] Erreur Excel Unifié: {e}")
        return None
