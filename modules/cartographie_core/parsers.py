...
def smart_fix_coords_france(lat: float, lon: float) -> Tuple[Optional[float], Optional[float]]:
    """
    Fix coordinate order for France (handles WFS axis order issues).
    
    Heuristic: If lat < 20 and lon > 35, assumes swapped coordinates.
    ...
    Returns:
        tuple: (lat, lon) corrected, or (None, None) if invalid.
    """
    try:
        val1 = float(lat)
        val2 = float(lon)
...
def parse_gml_coord_string(text: str, is_poslist: bool = False, dimension_hint: int = 2) -> List[List[float]]:
    """
    Parse GML coordinate strings into point lists.

    Args:
        text (str): The raw coordinate string.
        is_poslist (bool): True if space-separated list (GML posList), False if comma-separated (GML coordinates).
        dimension_hint (int): Expected dimension (2 or 3) for stride calculation.

    Returns:
        list: List of [lon, lat] points.
    """
    if not text: return []
    points = []
    try:
...
def parse_geometry_hybrid(element: ET.Element) -> Tuple[Optional[Dict[str, Any]], float, float]:
    """
    Parse various GML geometry formats (Point, LineString, Polygon) into GeoJSON.

    Args:
        element (ET.Element): The GML XML element containing geometry.

    Returns:
        tuple: (GeoJSON dict, center_lat, center_lon)
    """
    try:
        ...
        # --- SUPPORT LINES (LineString / Curve) ---
...
def parse_gml_response(xml_text: str) -> List[GeoFeature]:
    """
    Parse complete GML WFS response into a list of GeoFeature objects.

    Handles namespace stripping and both WFS 1.0 (featureMember) and 1.1 (featureMembers).

    Args:
        xml_text (str): The raw XML response string.

    Returns:
        List[GeoFeature]: A list of validated Pydantic feature objects.
    """
    rows = []
    try:
        # Simple namespace stripping for easier tag access
...
def parse_wfs_response(response_text: str, format_type: str = "json") -> List[Any]:
    """
    ...
    Returns:
        list: List of features (dicts or objects).
    """
    if format_type == "json":
        import json
        try:
