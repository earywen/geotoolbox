"""
PVA Image Processor - Advanced Georeferencing Module.

This module provides advanced image processing capabilities for historical
aerial photographs (PVA) to improve georeferencing accuracy:

1. Auto-Crop: Detect and remove black scanner borders
2. Rotation: Apply the orientation angle from IGN WFS metadata
3. Feature Matching: Align PVA with reference imagery using SIFT/ORB
...
def detect_photo_region(image: np.ndarray, threshold: int = 30) -> Optional[Tuple[int, int, int, int]]:
    """
    ...
    Args:
        ...
        threshold: Pixel intensity threshold for "black" (0-255)
        
    Returns:
        Tuple (x, y, width, height) of detected photo region, or None if failed
    """
    ...
    # Validate: photo should be at least 50% of image area
...
def crop_to_photo(image: np.ndarray, margin_percent: float = 1.0) -> Tuple[np.ndarray, Tuple[int, int, int, int]]:
    """
    Crop image to remove black scanner borders, keeping only the photo content.
    
    Args:
        image: Input image as numpy array
        margin_percent: Additional margin to keep around detected region (%)
...
def rotate_image(image: np.ndarray, angle_degrees: float, expand: bool = True) -> np.ndarray:
    """
    Rotate image by specified angle.
    
    The IGN WFS provides an 'orientation' property which represents the
    camera/aircraft heading during image capture.
...
def normalize_orientation(orientation: float) -> float:
    ...
    IGN orientation is typically the aircraft heading (0-360°, where 0 = North).
    To align the image with North up, we need to rotate by the negative of this.
    
    Args:
        orientation: IGN orientation value in degrees
        
    Returns:
        Rotation angle to apply for North-up alignment
    """
    # Normalize to 0-360 range
    orientation = orientation % 360
    
    # IGN orientation seems to be the viewing direction
    # For North-up, we rotate by negative orientation
    # But we need to account for typical aerial photo conventions
    
    # Most common: orientation 0 = photo aligned with North
    # So rotation needed = -orientation (or 360 - orientation)
...
def find_keypoints(image: np.ndarray, max_features: int = 1000) -> Tuple[Any, Any]:
    """
    Detect keypoints and compute descriptors using ORB.
    
    ORB is used instead of SIFT because:
    - It's free (no patent issues)
    - Faster computation
...
def match_images(img1: np.ndarray, img2: np.ndarray, 
    ...
    This aligns img1 (PVA) to img2 (reference mosaic) by finding common features.
    
    Args:
        img1: Source image (PVA to be aligned)
        img2: Reference image (modern mosaic)
        min_matches: Minimum number of good matches required
        
    Returns:
        3x3 homography matrix, or None if matching failed
    """
...
def align_to_reference(pva_image: np.ndarray, reference_image: np.ndarray,
    ...
    Args:
        pva_image: PVA image to align
        reference_image: Reference image (mosaic)
        output_size: Optional output size (width, height). Defaults to reference size.
        
    Returns:
        Aligned PVA image, or None if alignment failed
    """
...
def process_pva_image(
    ...
    3. Apply rotation based on IGN orientation (if enabled)
    4. Align to reference image (if provided)
    5. Save processed image
    
    Args:
        input_path: Path to input PVA TIF
        output_path: Path for processed output
        orientation: IGN orientation value in degrees
        reference_path: Optional path to reference image for alignment
        auto_crop: Whether to auto-crop black borders
...
def create_georeferenced_pva(
    ...
    This is the main entry point for improved PVA georeferencing. It:
    1. Crops black borders
    2. Applies rotation correction
    3. Recalculates the footprint based on cropping
    4. Creates accurate world file
    
    Args:
        input_path: Path to input PVA TIF
        footprint: Dict with min_lat, max_lat, min_lon, max_lon
        orientation: IGN orientation in degrees
        auto_crop: Whether to auto-crop
        apply_rotation: Whether to apply rotation
        
    Returns:
        Dict with results including new footprint
    """
    result = {
        "success": False,
        "input": input_path,
    ...
    try:
        ...
        if auto_crop:
            ...
            # The footprint represents the full image, we need to adjust
        ...
        if apply_rotation and orientation is not None:
            ...
            if abs(rotation_angle) > 1:
                image = rotate_image(image, rotation_angle)
                result["rotation_applied"] = rotation_angle
                # Note: rotation changes the footprint in complex ways
                # For now we keep the bbox, but mark that rotation was applied
